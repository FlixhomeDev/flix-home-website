    ...
    return (
        <div className="w-[252px] h-[520px] md:w-[380px] md:h-[740px] rounded-[10px] border border-[#DEE2E7] flex flex-col gap-5 bg-[#FCFCFC]">
            ...
            <div className="mt-0 md:mt-5 ml-5 w-[230px] md:w-full flex items-center gap-[10px]">
                <div className={`w-[60px] h-[23px] md:w-[73px] md:h-[36px] ${item.plan === "Avançado" ? "bg-[#ECAB2A]" : "bg-[#3C91E6]"} py-[9px] px-[13px] rounded-[8px] flex items-center justify-center`}>
                    <span className={`text-[8px] md:text-sm ${item.plan === "Avançado" ? "text-[#0E0E0E]" : "text-[#FFFFFF]"} font-medium font-inter`}>{item.plan}</span>
                </div>
                <span className="text-[11.34px] md:text-2xl text-[#3C91E6] font-medium font-inter">€ {item.priceByMonth}</span>
                <span className="text-[8.82px] md:text-lg text-[#3C91E6] font-medium font-inter">por / mês</span>
            </div>
            <div className="-mt-[6px] md:mt-[10px] ml-5 flex items-center gap-[3px]">
                <span className="text-[8.82px] md:text-lg text-[#6F6F6F] font-bold font-inter">€ {item.priceByYear}</span>
                <span className="text-[7.56px] md:text-base text-[#6F6F6F] font-medium font-inter">/Ano</span>
            </div>
            <span className="text-[8.82px] md:text-lg text-[#6F6F6F] font-normal font-inter -mt-[6px] md:mt-[10px] ml-5 w-auto md:w-[360px]">Ideal para quem quer segurança sem gastar muito!</span>
